...
.music-list {
  ...
  &-header {
    ...
    padding: 0 20/100rem;
    font-size: 30/100rem;
    border-bottom: 0.5px solid #cfc5c0;
    .iconfont {
      color: #999;
    }
    .icon-delete {
...
<template>
  <section class="music-list">
    <p class="music-list-mask" @click.stop="closeList" />
    <div class="music-list-wrap">
      <header class="music-list-header clearfix">
        <i class="iconfont icon-delete fr" @click="clearSong" />
        <p class="play-mode fl" @click="switchPlayMode">
          <i class="iconfont" :class="[playModeInfo.cls]" />{{
            playModeInfo.text
          }}
          <em v-if="playModeInfo.mode != 1">({{ musicList.length }}首)</em>
        </p>
      </header>
      <div class="music-list-body" ref="musicListRef">
        <ul v-if="musicList.length">
          ...
          >
            <p class="text-overflow" @click="playSong(song, index)">
              <i class="iconfont icon-shengyin" v-if="playIndex == index" />{{
                song.name
              }}
              - <span>{{ song.singers }}</span>
            </p>
            <i class="iconfont icon-close" @click="delSong(song, index)" />
          </li>
        </ul>
      ...
      <footer class="music-list-footer">
        <a href="javascript:;" @click="closeList">关闭</a>
      </footer>
    </div>
  </section>
</template>
