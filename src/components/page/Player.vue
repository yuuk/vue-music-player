...
.player-main {
  ...
  &-disc {
    position: relative;
    padding-top: 155/100rem;
    .disc-tonearm {
      transform-origin: 10% 10%;
      transition: all 0.5s ease-in-out;
      width: 330/1.5/100rem;
      height: 470/1.5/100rem;
      position: absolute;
      left: 340/100rem;
      top: 0;
      z-index: 3;
      background: url(../../assets/img/needle-plus.png) no-repeat center center;
      background-size: contain;
    }
    .disc-panel {
      position: relative;
      width: 1180/2/100rem;
      height: 1180/2/100rem;
      margin: auto auto;
      img {
        position: absolute;
        left: 0;
      ...
      &:before {
        ...
        width: 100%;
        height: 100%;
        background: url(../../assets/img/disc_light-plus.png) no-repeat center
          center;
        background-size: contain;
      }
      &:after {
        ...
        margin: auto;
        z-index: 1;
        background: url(../../assets/img/disc-plus.png) no-repeat center center;
        background-size: contain;
      }
    }
  }
  ...
  <div class="player-main">
    ...
    <header class="player-main-header">
      ...
      <p class="title text-overflow">
        {{ musicInfo.name }} <span class="singer">{{ musicInfo.singers }}</span>
      </p>
      <i class="share-icon iconfont icon-fenxiang" />
    </header>

    <div class="player-main-disc">
      <div class="disc-tonearm" />
    ...
    <footer class="player-main-footer">
      <div class="player-main-tools">
        <p>
          <i
            ...
            :class="{ 'icon-like1': isLiked }"
            @click="like"
          />
        </p>
        <p>
          <a :href="musicInfo.url" :download="musicInfo.name"
            ><i class="iconfont icon-LC_icon_download_line_5"
          /></a>
        </p>
        <p>
          <router-link :to="`/comment/music/${musicInfo.id}`">
      ...
      <div class="player-main-ctrl">
        <div class="timebar">
          <time class="played-time">{{
            musicInfo.currentTime | seconds2Time
          }}</time>
          <progress-bar
            :buffer="musicInfo.buffer"
            :percent="musicInfo.percent"
            @percentChange="onProgressChange"
          />
          <time class="total-time">{{
            musicInfo.duration | seconds2Time
          }}</time>
        </div>
        <div class="toolbar">
          <i
            class="iconfont"
            :class="[playModeInfo.cls]"
            @click="switchPlayMode"
          />
          <i class="iconfont icon-shangyiqu101" @click="switchMusic(-1)" />
          <i v-if="isLoading" class="iconfont icon-loading" />
          <i
            v-else
            class="iconfont"
            ...
            @click="togglePlay"
          />
          <i class="iconfont icon-xiayiqu101" @click="switchMusic(1)" />
          <i class="iconfont icon-liebiao" @click="togglePLaylist(true)" />
        </div>
      </div>
    </footer>
...
export default {
  ...
  methods: {
    ...
    togglePlay() {
      ...
      this.$refs.discWrapRef.style.transform =
        wrapTrans === 'none' ? discTrans : discTrans.concat('', wrapTrans);
    },
    //  切换播放模式
    switchPlayMode() {
      let min = 0;
