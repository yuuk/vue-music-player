  ...
  li {
    ...
    .cover {
      ...
      &:before {
        ...
        width: 40/100rem;
        height: 40/100rem;
        background: url('../../assets/img/icon-list-play.png') no-repeat 0 0;
      ...
      <ul class="clearfix">
        <li v-for="(item, index) in mvList" :key="index">
          <router-link :to="`/mv/${item.id}`">
            <p class="img">
              <img :src="item.cover" alt="" /><i
                class="iconfont icon-socialyoutubeoutline"
              />
            </p>
            <p class="title">
              {{ item.name }}
